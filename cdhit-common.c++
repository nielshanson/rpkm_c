...
void Sequence::PrintInfo( int id, FILE *fin, FILE *fout, const Options & options, char *buf )
{
	...
	fprintf( fout, "%i\t%s", id, identifier+1 );
	if( identity ){
		int *c = coverage;
		fprintf( fout, " at " );
	...
	while (not feof( fin ) || one.size) { /* do not break when the last sequence is not handled */
		...
		if( buffer[0] == '+' ){
			...
			if ( one.size ) { // write previous record
                ...
                //cout << "sequences.size = " << one.index << endl;
...
void SequenceDB::PrintSequences() {
	...
	for (i=0; i<N; i++) { 
       sorting[i] = ((long long)sequences[i]->index << 32) | i; 
       //printf("index : %lld Sorting[i] : %lld i : %d  Sorting : %lld Cluster_id : %d\n", (long long)sequences[i]->index, sorting[i], i,  sorting[i] & 0xffffffff, sequences[sorting[i] & 0xffffffff]->cluster_id);
       //printf("Size : %d\n",sequences[i]->size);

/*
       for(int j=0; j <sequences[i]->size; j++) {
          sequences[i]->SwapIn();
          if( sequences[i]->data!= NULL);
             printf("%c",aa[sequences[i]->data[j]]);
       }
       printf("\n");
*/
...
void SequenceDB::WriteExtra1D( const Options & options )
{
	...
	vector<long long> sorting( N );
    // in order to sort by index shift the index bits to higher bits
    // sort it by index and then decipher back by doing & operation with the 0xffffffff 
...
void SequenceDB::DoClustering( const Options & options )
{
	...
	for(i=0; i<N; ){
        //ratio of the total sequences examines with the representatives 
		float redundancy = (rep_seqs.size() + 1.0) / (i + 1.0);
		int m = i;
		size_t sum = 0;
#define TEST
